...
def policies_list_create(request):
    ...
    if request.method == 'POST':
        ...
        if PolicyTemplate.objects.filter(name=name).exists():
            return JsonResponse({'error': 'A policy template with this name already exists'}, status=409)
...
def credentials_list_create(request):
    ...
    if request.method == 'POST':
        ...
        if not template_type or claims_keys is None:
            return JsonResponse({'error': 'template_type and claims_keys are required'}, status=400)

        if CredentialTemplate.objects.filter(template_type=template_type).exists():
            return JsonResponse({'error': 'A credential template with this type already exists'}, status=409)
